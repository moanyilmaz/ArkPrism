...
Figure~\ref{fig:architecture} gives the end-to-end architecture. Project and rule inputs enter ArkAnalyzer for Scene, IR, and call-graph construction. \mytool then performs three connected analyses: (1)~API identity resolution computing $\mathit{Id}(v)$ and its witness, (2)~carrier propagation across IR and selected asynchronous continuations, and (3)~subgraph assembly that preserves the derivation label $\omega(e)$ on every evidence edge. Lifecycle ordering and call-graph context supply reachability evidence to this pipeline.
