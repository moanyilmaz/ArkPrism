...
\end{equation}
where $c=\mathit{Seed}(s,r_T)$ for an accepted, well-formed query rule and $c=\bot$ for detector-only evidence. This optional-carrier triple makes API recognition explicit without manufacturing a taint seed for operations whose sensitive value is not modeled.
...
\end{equation}
Here $\mathit{ArityCompat}$ requires exact arity whenever the rule declares a parameter array, including an empty one. $\mathit{ParamCompat}$ requires parameter names and types to match after normalizing imported type qualifiers, whitespace, array notation, callback aliases, and union order; rules with non-concrete return carriers are rejected before matching. Consequently, an unresolved call such as \texttt{avPlayer.on('error',...)} cannot match a configured geolocation or sensor event merely because both use \texttt{on} with the same arity. This predicate governs IFDS source seeding and is separate from the detector acceptance predicate above.
...
\mytool formalizes carrier binding as $\mathit{SrcBind}(call, cb, param)$ (Section~\ref{subsec:problem_def}). The five rules occupy two analysis domains. T1, T4, and T5 transfer source carriers inside IFDS; T2 and T3 recover callback context for the report graph without manufacturing taint facts. Table~\ref{tab:transfer_rules} makes this distinction explicit.
...
T5 & \texttt{async/await} & $fact(promise)\!\to\!await_{res}$ & Explicit \texttt{await} expression & May-path & IFDS normal flow \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{cases}
\{\mathit{Copy}(d,cb.param_0)\}, &
\mathit{Depends}(b,\mathit{val}(d)),\\
\emptyset, & \text{otherwise}.
\end{cases}
\end{equation}
The copied fact preserves $\epsilon(d)$ and records the \textsc{PromiseThen} derivation. ArkAnalyzer lowers both direct chaining and variable chaining to this relation: the configured source first taints the Promise temporary, after which the \texttt{then} call maps that fact into the resolved success handler. Rejection handlers and unrelated callback arguments receive no fact.
...
The normal-flow function therefore maps a fact on $promiseMethod()$ to $data$ and its subsequent uses. A bounded fallback remains available for definition-backed wrapper Promises whose executor-to-\texttt{await} relation is absent from ArkIR; such outcomes retain separate supplementary provenance and do not alter the IFDS fact relation.

\subsubsection{Algorithm}

Algorithm~\ref{alg:callback} gives the T4 extension to IFDS call flow. It executes at the \texttt{then} call edge, not after solver termination. The transfer is monotone: it only adds a source-preserving fact for the success parameter when receiver dependence and callback-position evidence hold.

\begin{algorithm}[H]
\caption{Continuation-aware IFDS call flow}
\label{alg:callback}
\footnotesize
\begin{algorithmic}[1]
...
\EndIf
\If{$d\neq\mathbf{0} \land
     (\Call{Depends}{s.base,\mathit{val}(d)} \lor
      \Call{MayAlias}{s.base,\mathit{val}(d)})$}
  \State $d' \gets \Call{CopyFact}{d,cb.param_0}$
...
Relative to the resolved ArkIR call edge and available alias relation, this transfer follows Promise success semantics: every accepted fact denotes a value that may be supplied as the first success-handler argument. The guard excludes \texttt{catch}, rejection callbacks, nonzero callback positions, and same-named \texttt{then} calls whose receiver is unrelated to the incoming fact. The source witness is copied rather than reconstructed, so source-distinct IFDS facts remain distinct across the continuation.
...
The report stores provenance at the level where it is produced. A call-chain relation carries a \texttt{callType}; a configured-query path carries $\tau(q)\in\{\mathit{ifds},\mathit{async\_supplement},\mathit{both}\}$; and an endpoint link records \texttt{exact\_statement} or \texttt{source\_location}. Detector-local sink edges are identified by their containing record. Evidence-graph assembly maps these fields to $\omega_u$ rather than replacing them with a synthetic global confidence score. Consequently, an asynchronous supplement cannot be presented as an IFDS path, and a normalized-location join remains distinguishable from an exact-statement join.
