...
Continuation transfer follows the same principle. T1 callbacks and Promise-\texttt{then} use construct-specific IFDS call flow, while explicit \texttt{await} is handled by native normal flow over ArkIR; each propagated fact retains its source witness and carrier state. T4 contributes exactly the nine Promise-success cases constructed to require that relation while leaving every matched negative unchanged. ArkPromiseBench further separates Promise ownership from method naming, success from rejection positions, and payload-dependent returns from constant or unrelated values. The optional unresolved-IR fallback remains separately labeled, so it cannot be mistaken for a solver-derived edge.
