...
Table~\ref{tab:hapbench_ci_app} reports Wilson intervals for the main configurations. The specificity intervals reflect HapBench's 14 negative cases; balanced accuracy and MCC summarize the positive and negative obligations jointly.
...
Oracle construction proceeds in two passes. Source-only classification first accepts an occurrence only when its executable context establishes the configured package/namespace/member and access kind through an import, SDK receiver type, constructor, manager/factory definition, or executable property read; detector reports are not consulted. It rejects comments, strings, declarations, same-name application methods, and unrelated library/controller receivers. Source/IR adjudication then resolves ambiguous aliases and lowered receiver forms without adding candidates. The gold set contains 90 accepted occurrences; its final decision record preserves all 186 judgments, including exact locations, evidence excerpts, reason codes, and 96 receiver- or owner-incompatible negatives. Validation checks every source location and canonical rule key and rejects duplicate occurrence identities. Evaluation joins detector and gold records on project, normalized file, exact line and column, canonical API, and access kind.
...
The path audit samples the complete set of \CorpusPaths{} corpus paths. A deterministic sampler balances source kind, provenance, sink family, path-length bin, and project diversity, producing 120 records from 47 projects. The final review queue contains 90 privacy-data and 30 framework-input paths; 64 IFDS, 28 supplementary, and 28 dual-provenance paths; and all available network and UI strata. Each record retains endpoint locations, the complete statement path, source and IR excerpts, source identity, provenance, and a decision reason. Exact rebinding preserves 118 reviewed paths in the final result set. Two deterministic replacements receive fresh source review, and two exact paths whose derivation changes from supplementary to dual provenance are rechecked against their Promise declarations.

Each referenced source/IR path receives five Boolean decisions: source identity, sink identity, explicit value dependence, reachability consistency, and provenance label. A path is accepted only when all five hold. The final audit confirms source identity, sink identity, and provenance for 120/120 paths; explicit dependence, reachability, and complete-path correctness hold for 115/120. All 90 privacy-data paths are complete. The five rejected framework-input paths join a lifecycle-written static URI to unrelated input fields, fixed validation messages, or an independent camera-picker result.

\subsection{Result Traceability}
\label{subsec:app_traceability}

The replication package provides ArkSourceFirst60's 186-item decision set and occurrence gold set; the 64-case ArkIdentityBench oracle; machine-readable sources, mutation operators, oracles, and configuration outputs for all 67 HapBench, 48 ArkAsyncBench, and 24 ArkPromiseBench cases; the Top-120 reproduction results; and the complete semantic-path audit. These records derive confusion matrices, construct/category metrics, endpoint-pair outcomes, paired tests, uncertainty intervals, and runtimes. Figure~\ref{fig:eval_hapbench} and Figure~\ref{fig:eval_corpus_landscape} are generated from evaluator JSON rather than transcribed values.

Corpus tables and figures use one validated result per project under a common analyzer build, SDK, rule set, and resource policy. Aggregation checks project inventory, completion state, source and sink endpoints, provenance, and detector-to-path links. Every run manifest records analyzer and frontend commits, source/sink/rule hashes, SDK fingerprint, continuation and fallback flags, memory budget, and timeout policy. Companion JSON records every plotted value and its evaluation configuration.
