...
function main() {
    ...
    const reportFiles = [];
    // Support both flat directory (all .json in reportsDir) and nested (project/project-report.json)
    const entries = fs.readdirSync(reportsDir, { withFileTypes: true });
    for (const entry of entries) {
        if (entry.isFile() && entry.name.endsWith('.json')) {
            reportFiles.push(path.join(reportsDir, entry.name));
        } else if (entry.isDirectory()) {
            const subDir = path.join(reportsDir, entry.name);
            try {
                const subEntries = fs.readdirSync(subDir);
                for (const sub of subEntries) {
                    if (sub.endsWith('.json')) {
                        reportFiles.push(path.join(subDir, sub));
                    }
                }
